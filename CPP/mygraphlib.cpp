...
// Uses neato to generate an image for the CPP solution
int CreateCPPGraph(Digraph &g, DNodePosMap &px, DNodePosMap &py, vector<Arc> &solution, DNodeIntMap &components)
{
    ...
    factor = 100; // quanto maior, menor o desenho do vértice
    ...
    vector<Color> colors{BLACK,RED,GREEN,BLUE,MAGENTA,CYAN,GRAY,ORANGE};

    // set nodes info
    if(solution.size() > 0){
        for (DNodeIt v(g); v!=INVALID; ++v) {
            myfile << "\t" << "\"" << to_string(g.id(v)).c_str() << "\"";
            myfile << "[style = \"bold\", color=\"" << ColorName(BLACK) << "\", pos = \""
        ...
        for (DNodeIt v(g); v!=INVALID; ++v) {
            myfile << "\t" << "\"" << to_string(g.id(v)).c_str() << "\"";
            myfile << "[style = \"bold\", color=\"" << ColorName(colors[components[v] % 8]) << "\", pos = \""
                   << factor*(px[v]-minpx)/delta << "," << factor*(py[v]-minpy)/delta << "!\" ];\n";
        }
    }
    ...
    if(solution.size() > 0){
        for(int i = 0; i < solution.size(); i++){
            myfile << "\t\"" << to_string(g.id(g.source(solution[i]))).c_str() << "\" -> \"" <<
                                to_string(g.id(g.target(solution[i]))).c_str() << "\"" <<
                                "[label = \"" << i << "\" color=\"" << ColorName(BLACK) << "\" style=solid];\n";
        }
    }
    else{
        for(ArcIt a(g); a != INVALID; ++a){
            myfile << "\t\"" << to_string(g.id(g.source(a))).c_str() << "\" -> \"" <<
                                to_string(g.id(g.target(a))).c_str() << "\"" <<
                                "[color=\"" << ColorName(BLACK) << "\" style=solid];\n";
        }
    }
